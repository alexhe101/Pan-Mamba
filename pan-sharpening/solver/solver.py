...
class Solver(BaseSolver):
    ...
    def train(self): 
        with tqdm(total=len(self.train_loader), miniters=1,
            ...
            k = -0.3
            b=0.001
            para = max(0.0001,k*(self.epoch-1)+b)
            # para=0
            # para = 0.0001
            # para = 0.001
            # gate_cof = max(1,k*(self.epoch-1)+100)
            gate_cof =100
            # para = 1
            # para=0
            epoch_loss = 0
            # if self.epoch>300:
            #     for pm in self.model.module.maskp.parameters():
            ...
            for iteration, batch in enumerate(self.train_loader, 1):
                ...
                if self.cuda:
                    ...
                    mask_label = bms_image
                self.optimizer.zero_grad()
                self.model.train()
                # y = self.model(lms_image, bms_image, pan_image)
                y, mask,gate = self.model(lms_image, bms_image, pan_image)
                #y, mask = self.model(lms_image, bms_image, pan_image)

                # print(torch.min(abs(y-bms_image)))
                # print(torch.max(abs(y-bms_image)))
                #
                lfgate,hfgate,decgate = gate
                lf_gateloss = gate_loss(lfgate)
                hf_gateloss =gate_loss(hfgate)
                dec_gateloss = gate_loss(decgate)
                # # #
                gl = lf_gateloss+hf_gateloss+dec_gateloss
                # # gl = lf_gateloss+hf_gateloss

                # # gl = gl/(gl.detach()+ np.finfo(np.float64).eps)
                # gl = 0
                # print(mask.size())
                # print(mask_label.size())
                # ce_loss = self.ce(mask,mask_label) / (self.cfg['data']['batch_size'] * 2)

                # ce_loss = self.ce(mask,mask_label) / (self.cfg['data']['batch_size'] * 2)
                # ce_loss=0
                # ce_loss = ce_loss/(ce_loss.detach()+ np.finfo(np.float64).eps)
                # mask_loss+=ce_loss.data
                spa_loss = self.loss(y, ms_image) / (self.cfg['data']['batch_size'] * 2)
                # spa_loss = spa_loss/(spa_loss.detach()+np.finfo(np.float64).eps)
                ce_loss =  self.loss(50*torch.ones_like(mask).cuda(), mask) / (self.cfg['data']['batch_size'] * 2)
                # ce_loss = 0
                loss = self.loss(y, ms_image) / (self.cfg['data']['batch_size'] * 2) + para * ce_loss+gate_cof*gl
                # loss = self.loss(y, ms_image) / (self.cfg['data']['batch_size'] * 2)+gate_cof*gl

                # loss = self.loss(y, ms_image) / (self.cfg['data']['batch_size'] * 2)
                # loss = spa_loss+para*ce_loss+0.1*gl
                # gate_sum = gate_sum+lf_gateloss+hf_gateloss+dec_gateloss
                # total_lowgate+=lfgate
                # total_highgate+=hfgate
                # total_decgate+=decgate
            ...
            # print(gate_sum)
            # print(torch.sum(total_lowgate,dim=0))
            # print(torch.sum(total_highgate,dim=0))
            # print(torch.sum(total_decgate,dim=0))
            # # print(mask)
            # # print(mask_loss)
            # epoc = str(self.epoch)
            # for i in range(4):
            #     feature_save(mask[i], os.path.join("WV2mask_epoch",epoc,str(i)))
            #     # feature_save(mask_label[i], os.path.join("mask_label",epoc,str(i)))
    def eval(self):
    ...
    def save_checkpoint(self,epoch):
        ...
        if self.cfg['save_best']:
            ...
            # if self.records['QNR'] !=[] and self.records['QNR'][-1]>0.85 and self.records['PSNR'][-1]>49:
            #     shutil.copy(os.path.join(self.cfg['checkpoint'] + '/' + str(self.log_name), 'latest.pth'),
            #                 os.path.join(self.cfg['checkpoint'] + '/' + str(self.log_name), 'bestGF.pth'))
            # # if epoch%25==0:
            #     s = str(epoch)+".pth"
            #     print(s)
            #     shutil.copy(os.path.join(self.cfg['checkpoint'] + '/' + str(self.log_name), 'latest.pth'),
            #                 os.path.join(self.cfg['checkpoint'] + '/' + str(self.log_name), s))
    def run(self):
        self.check_gpu()
